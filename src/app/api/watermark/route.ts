...
// Helper to overlay fullsizepost.online watermark on an image buffer
async function addWatermark(imgBuffer: Buffer): Promise<Buffer> {
  ...
  const textWidth = Math.round(fontSize * 9.5);
...
// POST method: Backwards compatibility (returns base64 dataUrl)
